...
DPEs_eV = (BCB_Es);%bcb_finds;%rfc_Es_after;%(rfc_FL_Es - rfc_small_cut_Es)*pix2eV;
...
E_interval_list = [460,480; 480,500; 500,510; 510,520; 520,535; 535,555];% 495,500; 505,510; 510,525; 525,540; 540,555];%[320,340; 340,350; 350,355; 355,360; 360,380];
B_interval_list = (E_interval_list - min(B_map(:)))/(max(B_map(:))-min(B_map(:)));

pre_filter = cat(1,[]);
pre_filter(1,:) = [170,189, kLOS];
...
[E_interp,K_interp] = meshgrid(-299.5:.5:250, -80:.5:80); 
...
nn=0;
nnn=0;
out_spec = zeros(107,130,size(region_list,1));
out_kax = zeros(97,1,size(region_list,1));
out_eax = zeros(1,130,size(region_list,1));
...
for iii=1:size(region_list,1)    
    ...
    out_spec(:,:,iii) = region_arpes_binned;
    out_kax(:,:,iii) = Kaxis_invA;
    out_eax(:,:,iii) = Eaxis_eV_0fixed;
    out_nnn(iii) = nnn;
end
