...
table_title = 'BCB fit';
dirac_Es = dos_Es;%502*ones(1,961);% BCB_Es;%BCB_Es;%rfc_Es_after;   %input Energies vector (cone pixels)
dirac_ks = dos_Ks;%181*ones(1,961);%kLOS;%181*ones(1,961);% kLOS;%.5*(rfc_ks_after+kLOS);   %input mtm vector (cone pixels)

DPEs_eV = (AVE_FL - dos_Es)*pix2eV;%(rfc_FL_Es - bcb_findss)*pix2eV;%(645*ones(1,961)-dirac_Es)*pix2eV;%(rfc_FL_Es - bcb_findss)*pix2eV;%bcb_finds;%rfc_Es_after;%(rfc_FL_Es - rfc_small_cut_Es)*pix2eV;
B_map = reshape(DPEs_eV,31,31);   

B_interval_list_abs = [.462,.4674; .4674,.4701; .4701,.4728; .4728,.4782; .4782,.4836; .4836,.489];%[.2,.21; .21,.22; .22, .23; .24,.25; .25,.26; .26,.27];%[.22,.225; .225,.23; .23,.235; .235,.24; .24,.25; .25,.26];%[.22,.2236; .2236,.226; .2271,.2283; .2283,.233];%[400,500; 500,600];%[0,.5; .5,1];%.195,.205; .205,.215; .215,.225; .225,.235; .235,245];%; 480,500; 500,510; 510,520; 520,535; 535,555];% 495,500; 505,510; 510,525; 525,540; 540,555];%[320,340; 340,350; 350,355; 355,360; 360,380];
...
[E_interp,K_interp] = meshgrid(-399:1:550, -100:1:100); 
