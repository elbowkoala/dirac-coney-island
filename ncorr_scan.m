...
padding = 2*5;
draw_box = zeros([71+padding],[80+padding]);
E_0 = 30 + padding/2;
...
nEs = zeros(1,num_scans);
nks = zeros(1,num_scans);
nAs = zeros(1,num_scans);
nBs = zeros(1,num_scans);
ncorrs = zeros(1,num_scans);
ncorrspreads = zeros(1,num_scans);
...
for i = scan_is
    ...
    for AB_IT_i = 1:(length(A_range)*length(B_range))
        ...
        for x = 1:(size(f_image,2) - size(IT_template,2) +1)
            for y = 1:(size(f_image,1) - size(IT_template,1) +1)
                f_mean = mean( sum( dot( IT10_temp, f_image(y:y+size(IT_template,1)-1, x:x+size(IT_template,2)-1))));
                t_denom = 0;
                f_denom = 0;
        ...
        if AB_corr_table(AA_i,BB_i) == max(AB_corr_table(AB_corr_table~=0))
            corr_IT = normcorr;
            AA_it_i = AA_i;
            BB_it_i = BB_i;
            AB_IT_i_it = AB_IT_i;
    ...
    nEs(i) = DPE_pix;
    nks(i) = DPK_pix;
    nAs(i) = A_range(AA_it_i);
    nBs(i) = B_range(BB_it_i);
    ncorrs(i) = max(corr_IT(:));
    ncorrspreads(i) = corrspread;
    ...
    if wannasee == 1
        ...
        title({['spread=',num2str(ncorrspreads(i))];['maxC=',num2str(ncorrs(i))]})

        subplot(2,2,3), 
        imagesc((1:size(f_image,1)),fliplr(1:size(f_image,2)),rot90(f_image,1)), axis xy
        title(['DPI=',num2str(round(DPI_big(i)))])

        subplot(2,2,4), imagesc((1:size(f_image,1)),fliplr(1:size(f_image,2)),rot90(f_image+1.5*overlay,1)), axis xy, hold on;
