...
padding = 2*5;
draw_box = zeros([71+padding],[41+padding]);
E_0 = 40 + padding/2;
K_0 = round(size(draw_box,1)/2);
draw_x = (1:size(draw_box,1))';
draw_x0 = draw_x - K_0;

A_range = [10:10:30];%1.0];%[1.3:.05:2.0];%[1.3:.1:2.5];%[0.5:.5:3.0];%[.6:.2:2];%[1.8];%[1,2,3];
B_range =  [.025:.025:.075];%[0:.002:.01];%[0,.05,.1];
...
for i = scan_is
    ...
    b_E_1 =  round(rfc_FL_Es(i)/bin_E + bin_E/2)-40;
    ...
    fass_gg = fass_g;
    fass_gg(fass_gg>(mean(fass_g(:))+1*std(fass_g(:)))) = mean(fass_g(:))+1*std(fass_g(:));
    f_image = fass_gg; 
